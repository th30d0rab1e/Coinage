-- Recover filled BUY orders that have no position row.
-- Catches the case where a buy fills but the position row was missing
-- (deleted externally, or row never existed due to earlier bugs).
-- Without this, filled crypto holdings sit untracked with no sell order.
