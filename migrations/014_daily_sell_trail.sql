-- Add trailing branch for daily_sell positions in vw_edit_orders.
-- Previously daily_sell = true was excluded from all sell branches, so the
-- 1% tight stop placed by processDailyProfit() never trailed upward.
-- This branch trails it up whenever price rises more than 1% above the stop.
...
-- SELL daily_sell: trail at 1% below current price
SELECT p.name, p.period_type,
    ...
    TRUNC(s.price::numeric * 0.99, s.price_rounding) AS new_stop_price, 'sell'::text AS order_type,
...
WHERE p.sell_coinbase_order_id IS NOT NULL AND p.sell_filled_price IS NULL
  ...
  AND p.sell_stop_price < TRUNC(s.price::numeric * 0.99, s.price_rounding)::double precision
