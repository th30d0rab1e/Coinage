-- Reconcile cancelled orders: if a position's coinbase_order_id is no longer
-- in bulk_open_orders, the order was cancelled (by user or externally).
-- NULL out the ID so processBuyOrders/processSellOrders recreates it next cycle.
-- Filled orders are excluded via buy_filled_price IS NULL / sell_filled_price IS NULL
-- since fills are processed earlier in thee_procedure.
