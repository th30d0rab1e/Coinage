-- Fix buy INSERT blocking condition: use buy_order_id IS NOT NULL instead of
-- buy_coinbase_order_id IS NOT NULL in the LEFT JOIN. This ensures that when
-- reconciliation temporarily NULLs buy_coinbase_order_id (cancelled order),
-- the position still blocks new buys for that coin+period because buy_order_id
-- always remains set on existing positions.
