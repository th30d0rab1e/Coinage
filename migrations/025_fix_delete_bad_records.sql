-- Fix infinite duplicate loop: "delete bad records" was deleting positions
-- whose buy_coinbase_order_id was temporarily NULLed by reconciliation after
-- processBuyOrders set error_message on a failed placement. This caused the
-- buy INSERT to see no blocking position and create a new one each cycle.
--
-- Fix: clear error_message instead of deleting, so processBuyOrders retries
-- the order next cycle and the position stays in place as a buy blocker.
