-- Two changes:
-- 1. Sell stops floored at buy_filled_price so the limit price is always
--    above the buy price (no guaranteed-loss exits).
-- 2. Buy condition simplified: only buy if no open (unfilled) buy order
--    exists for that coin + period_type combination.
...
-- vw_edit_orders: floor new_stop_price and order_price at buy_filled_price
-- so trailing remakes never move the stop below break-even.
...
-- SELL trailing up: stop is below the vol-adjusted target (floored at buy_filled_price)
SELECT p.name, p.period_type,
    GREATEST(p.buy_filled_price::numeric,
        TRUNC(s.price::numeric * (vol.stop_ratio + p.sell_counter::numeric * 0.001) * 0.99, s.price_rounding)
    ) AS order_price,
    s.price AS price_now, p.buy_order_id, p.sell_coinbase_order_id AS coinbase_order_id, p.shares,
    GREATEST(p.buy_filled_price::numeric,
        TRUNC(s.price::numeric * (vol.stop_ratio + p.sell_counter::numeric * 0.001), s.price_rounding)
    ) AS new_stop_price, 'sell'::text AS order_type,
    TRUNC((GREATEST(p.buy_filled_price::numeric,
        TRUNC(s.price::numeric * (vol.stop_ratio + p.sell_counter::numeric * 0.001), s.price_rounding)
    ) - p.buy_filled_price::numeric) * p.shares::numeric, 2) AS estimated_profit,
    p.sell_counter AS counter
FROM position p
JOIN stock s ON p.stock_id = s.stock_id
...
WHERE p.sell_coinbase_order_id IS NOT NULL AND p.sell_filled_price IS NULL
  ...
  AND p.sell_stop_price < GREATEST(p.buy_filled_price::numeric,
        TRUNC(s.price::numeric * (vol.stop_ratio + p.sell_counter::numeric * 0.001), s.price_rounding))::double precision
