-- Prevent orphan recovery from creating duplicate unfilled buy positions.
-- Adds NOT EXISTS check: only recover an orphaned Coinbase order if there is
-- no existing unfilled buy position for that coin + period_type already.
