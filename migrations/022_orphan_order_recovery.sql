-- Orphan order recovery: if a BUY order exists on Coinbase (bulk_open_orders)
-- but has no matching position record, create one automatically.
-- Period type is inferred from order value: <$5 = day, $5-$50 = month, >$50 = year.
-- This runs at the top of thee_procedure before any buy/sell logic so the
-- position is tracked before the rest of the cycle acts on it.
